...
public class MyFirewall implements IOFMessageListener, IFloodlightModule {
	...
	public static String ColumnNames[] = { COLUMN_RULEID, COLUMN_DPID, COLUMN_IN_PORT, COLUMN_DL_SRC, COLUMN_DL_DST,
			COLUMN_DL_TYPE, COLUMN_NW_SRC_PREFIX, COLUMN_NW_SRC_MASKBITS, COLUMN_NW_DST_PREFIX, COLUMN_NW_DST_MASKBITS,
			COLUMN_NW_PROTO, COLUMN_TP_SRC, COLUMN_TP_DST, COLUMN_WILDCARD_DPID, COLUMN_WILDCARD_IN_PORT,
			COLUMN_WILDCARD_DL_SRC, COLUMN_WILDCARD_DL_DST, COLUMN_WILDCARD_DL_TYPE, COLUMN_WILDCARD_NW_SRC,
			COLUMN_WILDCARD_NW_DST, COLUMN_WILDCARD_NW_PROTO, COLUMN_PRIORITY, COLUMN_ACTION };
	 ...
	 * 
	 * Similar to getStorageRules(), which only reads contents for REST GET and does
	 * no parsing, checking, nor putting into FirewallRule objects
	 * 
				...
				try {
					...
					for (String key : row.keySet()) {
						...
						if (key.equals(COLUMN_RULEID) || key.equals(COLUMN_DPID) || key.equals("id")) {
							...
							r.nw_src_prefix_and_mask = IPv4AddressWithMask.of(
									IPv4Address.of(Integer.parseInt((String) row.get(COLUMN_NW_SRC_PREFIX))),
									r.nw_src_prefix_and_mask.getMask());
						} else if (key.equals(COLUMN_NW_SRC_MASKBITS)) {
							r.nw_src_prefix_and_mask = IPv4AddressWithMask.of(r.nw_src_prefix_and_mask.getValue(),
									IPv4Address.of(Integer.parseInt((String) row.get(COLUMN_NW_SRC_MASKBITS))));
						} else if (key.equals(COLUMN_NW_DST_PREFIX)) {
							r.nw_dst_prefix_and_mask = IPv4AddressWithMask.of(
									IPv4Address.of(Integer.parseInt((String) row.get(COLUMN_NW_DST_PREFIX))),
									r.nw_dst_prefix_and_mask.getMask());
						} else if (key.equals(COLUMN_NW_DST_MASKBITS)) {
							r.nw_dst_prefix_and_mask = IPv4AddressWithMask.of(r.nw_dst_prefix_and_mask.getValue(),
									IPv4Address.of(Integer.parseInt((String) row.get(COLUMN_NW_DST_MASKBITS))));
	...
	public void enableFirewall(boolean enabled) {
		if (this.enabled != enabled) {
			...
			List<Masked<U64>> changes = ImmutableList.of(Masked.of(DEFAULT_COOKIE, AppCookie.getAppFieldMask()));
	...
	public synchronized void deleteRule(int ruleid) {
		...
		Masked<U64> delDescriptor = Masked.of(AppCookie.makeCookie(APP_ID, ruleid),
	...
	/**
	 * Iterates over the firewall rules and tries to match them with the incoming
	 * packet (flow). Uses the FirewallRule class's matchWithFlow method to perform
	 * matching. It maintains a pair of wildcards (allow and deny) which are
	 * assigned later to the firewall's decision, where 'allow' wildcards are
	 * applied if the matched rule turns out to be an ALLOW rule and 'deny'
	 * wildcards are applied otherwise. Wildcards are applied to firewall decision
	 * to optimize flows in the switch, ensuring least number of flows per firewall
	 * rule. So, if a particular field is not "ANY" (i.e. not wildcarded) in a
	 * higher priority rule, then if a lower priority rule matches the packet and
	 * wildcards it, it can't be wildcarded in the switch's flow entry, because
	 * otherwise some packets matching the higher priority rule might escape the
	 * firewall. The reason for keeping different two different wildcards is that if
	 * a field is not wildcarded in a higher priority allow rule, the same field
	 * shouldn't be wildcarded for packets matching the lower priority deny rule
	 * (non-wildcarded fields in higher priority rules override the wildcarding of
	 * those fields in lower priority rules of the opposite type). So, to ensure
	 * that wildcards are appropriately set for different types of rules (allow vs.
	 * deny), separate wildcards are maintained. Iteration is performed on the
	 * sorted list of rules (sorted in decreasing order of priority).
	 * 
	 * @param sw   the switch instance
	 * @param pi   the incoming packet data structure
	 * @param cntx the floodlight context
	 * @return an instance of RuleWildcardsPair that specify rule that matches and
	 *         the wildcards for the firewall decision
	 */
	protected RuleMatchPair matchWithRule(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		synchronized (rules) {
			...
			while (iter.hasNext()) {
				...
				if (rule.matchesThisPacket(sw.getId(), (pi.getVersion().compareTo(OFVersion.OF_12) < 0 ? pi.getInPort()
						: pi.getMatch().get(MatchField.IN_PORT)), eth, adp) == true) {
					matched_rule = rule;
					break;
				}
			}
		...
		if (matched_rule == null) {
			/*
			 * No rule was found, so drop the packet with as specific of a drop rule as
			 * possible as not to interfere with other firewall rules.
			 */
			Match.Builder mb = OFFactories.getFactory(pi.getVersion()).buildMatch();
			mb.setExact(MatchField.IN_PORT,
			...
			if (mb.get(MatchField.ETH_TYPE).equals(EthType.IPv4)) {
				...
				if (mb.get(MatchField.IP_PROTO).equals(IpProtocol.TCP)) {
					...
					mb.setExact(MatchField.TCP_SRC, tcp.getSourcePort()).setExact(MatchField.TCP_DST,
							tcp.getDestinationPort());
				} else if (mb.get(MatchField.IP_PROTO).equals(IpProtocol.UDP)) {
					UDP udp = (UDP) ipv4.getPayload();
					mb.setExact(MatchField.UDP_SRC, udp.getSourcePort()).setExact(MatchField.UDP_DST,
			...
			rmp.match = mb.build();
			// rmp.match = adp.drop.build(); This inserted a "drop all" rule if no match was
			// found (not what we want to do...)
	...
	/**
	 * Checks whether an IP address is a broadcast address or not (determines using
	 * subnet mask)
	 * 
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision,
		...
		if (eth.isBroadcast() == true) {
			...
			// the case to determine if we have L2 broadcast + L3 unicast (L3 broadcast
			// default set to /24 or 255.255.255.0)
			// don't allow this broadcast packet if such is the case (malformed packet)
			if ((eth.getPayload() instanceof IPv4)
					&& !isIPBroadcast(((IPv4) eth.getPayload()).getDestinationAddress())) {
		...
		/*
		 * ARP response (unicast) can be let through without filtering through rules by
		 ...
		 * decision.setDescriptor(ALLOW_BCAST_COOKIE); decision.addToContext(cntx);
		 * return Command.CONTINUE; }
		 */

		// check if we have a matching rule for this packet/flow and no decision has
		// been made yet
		if (decision == null) {
			...
			// Drop the packet if we don't have a rule allowing or dropping it or if we
			// explicitly drop it
